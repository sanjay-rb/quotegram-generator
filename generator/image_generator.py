...
def generate_image_from_quote(quote_data: dict) -> str:
    try:
        ...
        background_prompt = (
            "inspirational cinematic abstract background, "
            "vibrant colors, 4k, modern artstation style, "
            "soft cinematic lighting, subtle gradients and texture, "
            "large clean empty space at the top and on the left, "
            "no text, no quote, no author name, no words, no letters, "
            "no numbers, no logos, no watermarks, no signatures, "
            "no UI, no icons, no symbols"
        )
        print(f"Generating image for prompt: {background_prompt}")
